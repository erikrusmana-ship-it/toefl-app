...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('peserta')
    .select('id,nama,npm,prodi,email,raw_listening,scaled_listening,raw_structure,scaled_structure,raw_reading,scaled_reading,skor_akhir,cefr_level,status_tes,pelanggaran_count,created_at,submitted_at')
    .order('created_at', { ascending: false })

  if (error) return Response.json({ error: error.message }, { status: 500 })

  const headers = ['ID','Nama Lengkap','NPM','Prodi','Email','Raw Listening','Scaled Listening','Raw Structure','Scaled Structure','Raw Reading','Scaled Reading','Total Score','CEFR','Status','Pelanggaran','Mulai','Selesai']
  const rows = (data || []).map((participant) => [participant.id,participant.nama,participant.npm,participant.prodi,participant.email,participant.raw_listening,participant.scaled_listening,participant.raw_structure,participant.scaled_structure,participant.raw_reading,participant.scaled_reading,participant.skor_akhir,participant.cefr_level,participant.status_tes,participant.pelanggaran_count,participant.created_at,participant.submitted_at].map(csvValue).join(','))
