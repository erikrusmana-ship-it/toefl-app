...
export class ResultEmailConfigurationError extends Error {}
...
export function isValidResultEmailPayload(payload: Partial<ResultEmailPayload>): payload is ResultEmailPayload {
  const result = payload.result
  const totals = payload.questionTotals
  const submittedAtIsValid = payload.submittedAt === undefined
    || (typeof payload.submittedAt === 'string' && !Number.isNaN(Date.parse(payload.submittedAt)))
  ...
  return Boolean(
    ...
    && submittedAtIsValid && violationsAreValid
    && (payload.statusTes === 'selesai' || payload.statusTes === 'dihentikan_pelanggaran')
  )
}

function formatJakartaDate(value: string | undefined, dateStyle: 'full' | 'medium' = 'full') {
  const date = value ? new Date(value) : new Date()
  return date.toLocaleString('id-ID', {
    timeZone: 'Asia/Jakarta',
    dateStyle,
    timeStyle: dateStyle === 'full' ? 'long' : 'medium',
  })
}

function buildResultEmailHtml(payload: ResultEmailPayload) {
  const { nama, npm, prodi, email, packageName, result, questionTotals, violations, statusTes } = payload
  const submittedAt = formatJakartaDate(payload.submittedAt)
  const testStatusLabel = statusTes === 'dihentikan_pelanggaran'
    ? 'Dihentikan otomatis karena pelanggaran kedua'
    : 'Selesai normal'
  const violationRows = violations.map((violation, violationIndex) => `<tr>
      ...
      <td style="padding:8px;border:1px solid #ddd">${escapeHtml(formatJakartaDate(violation.occurredAt, 'medium'))}</td>
    </tr>`).join('')
...
export async function sendResultEmail(payload: ResultEmailPayload) {
  if (!isValidResultEmailPayload(payload)) {
    throw new ResultEmailDeliveryError('Data hasil tidak valid.', 400)
  }

  const apiKey = process.env.RESEND_API_KEY
  if (!apiKey) {
    throw new ResultEmailConfigurationError('RESEND_API_KEY belum dikonfigurasi.')
  }
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    headers: {
      ...
      'Idempotency-Key': `toefl-result-${payload.participantId}`,
    },
    body: JSON.stringify({
      from: process.env.RESULT_EMAIL_FROM || 'TOEFL UNPAS <onboarding@resend.dev>',
      to: [RESULT_RECIPIENT],
      reply_to: payload.email,
      subject: `${payload.statusTes === 'dihentikan_pelanggaran' ? '[PELANGGARAN] ' : ''}Hasil English Proficiency Test — ${payload.nama} (${payload.npm})`,
  ...
  if (!response.ok) {
    const detail = JSON.stringify(responseBody).slice(0, 350)
    throw new ResultEmailDeliveryError(`Resend ${response.status}: ${detail}`, response.status)
  }
